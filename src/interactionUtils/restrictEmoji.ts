...
export default class RestrictEmojiUtils {
    ...
    async submitRoles(interactionData: RestrictEmojiInteraction, newInteraction: Interaction) {
        ...
        if (!inter.guild_id) {
            this.bot.logger.debug('PENDING INTERACTIONS', 'Restrict emoji: Initial interaction is missing guild_id:', inter);
            return;
        }
        const values = newInteraction.data?.values as Snowflake[] | undefined;
        if (!values?.length) {
            this.bot.logger.debug('PENDING INTERACTIONS', 'Restrict emoji interaction is missing values:', newInteraction);
            return;
        }
        let respText = '';
        await this.bot.api.emoji.edit(inter.guild_id, interactionData.emoji.id, { roles: values }).then(newEmoji => {
            if (newEmoji) {
                respText = LangUtils.getAndReplace('REM_SET_ROLES', {
                ...
                respText = LangUtils.getAndReplace('REM_SET_ROLES_UNNECESSARY', { emoji: interactionData.emojiText }, inter.locale);
                respText = `${this.bot.utils.getEmoji('SUCCESS_ALT', inter)} ${respText}`;
            }
        }).catch(err => {
            this.bot.logger.handleError('/restrict-emoji', err);
            respText = LangUtils.getAndReplace('REM_EDIT_FAILED', { emoji: interactionData.emojiText }, inter.locale);
