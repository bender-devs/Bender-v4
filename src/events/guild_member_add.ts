...
export default class GuildMemberAddHandler extends EventHandler<GuildMemberAddData> {
    ...
    handler = async (eventData: GuildMemberAddData) => {
        ...
        if (!guild) {
            this.bot.logger.handleError('guild_member_add', 'Not performing member actions because guild fetch failed!');
            return null;
        }
        if (settings.memberLog?.channel && settings.memberLog?.join) {
            const replacedMessage = Replacers.welcomeMessage(settings.memberLog.join, eventData.user, guild, true, guild.preferred_locale);
        ...
        if (settings.memberLog?.joinDM) {
            const replacedMessage = Replacers.welcomeMessage(settings.memberLog.joinDM, eventData.user, guild, false, guild.preferred_locale);
