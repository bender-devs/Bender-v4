...
export default async function (this: FunCommand, interaction: Interaction, choices?: CommandOptionValue) {
    ...
    if (choicesArr.length < 2) {
        ...
        return this.respond(interaction, `${oneChoice} (${hint})`, 'THINK').catch(this.handleAPIError.bind(this));
    }
    choicesArr = choicesArr.filter((item, index) => choicesArr.indexOf(item) === index); // remove duplicates
    if (choicesArr.length < 2) {
        return this.respond(interaction, oneChoice, 'THINK').catch(this.handleAPIError.bind(this));
    }
    ...
    return this.respond(interaction, choiceText, 'THINK').catch(this.handleAPIError.bind(this));
}
