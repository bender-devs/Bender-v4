...
export default async function (
    ...
    count?: CommandOptionValue,
    sides?: CommandOptionValue
) {
    ...
    if (count === 1) {
        ...
        if (sides === 6) {
            ...
            return this.respond(interaction, `${outcomePrepend} ${emojiText} **${num}**`, 'DIE').catch(
                this.handleAPIError.bind(this)
            );
        }

        const outcomeText = LangUtils.getAndReplace('FUN_DICE_SIDES_SINGLE', { num, sides }, interaction.locale);
        return this.respond(interaction, outcomeText, 'DIE').catch(this.handleAPIError.bind(this));
    }
    ...
    let sum = 0;
    let reply = LangUtils.getAndReplace(
        `FUN_DICE_${sides === 6 ? 'NORMAL' : 'SIDES'}`,
        { count, sides },
        interaction.locale
    );
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (results[i] > 0) {
            ...
            reply += LangUtils.getAndReplace(
                `FUN_DICE_DETAIL${results[i] === 1 ? '_SINGLE' : ''}`,
                { num: i + 1, count: results[i] },
                interaction.locale
            );
        }
    }
    ...
    return this.respond(interaction, reply, 'DIE').catch(this.handleAPIError.bind(this));
}
