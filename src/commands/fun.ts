    ...
    run(interaction: Interaction): CommandResponse {
        ...
        switch (subcommand) {
            ...
            case LangUtils.get('FUN_DICE_SUBCOMMAND'): {
                const numDice = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_DICE_OPTION_COUNT')
                )?.value;
                const numSides = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_DICE_OPTION_SIDES')
                )?.value;
                return dice.bind(this)(interaction, numDice, numSides);
            }
            case LangUtils.get('FUN_RPS_SUBCOMMAND'): {
                const show = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_RPS_OPTION')
                )?.value;
                const user = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_RPS_OPTION_USER')
                )?.value;
            ...
            case LangUtils.get('FUN_RANDOM_SUBCOMMAND'): {
                const min = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_RANDOM_OPTION_MIN')
                )?.value;
                const max = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_RANDOM_OPTION_MAX')
                )?.value;
                const dec = args?.[0]?.options?.find(
                    (opt) => opt.name === LangUtils.get('FUN_RANDOM_OPTION_DEC')
                )?.value;
