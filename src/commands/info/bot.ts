...
export default async function (this: InfoCommand, interaction: Interaction) {
    ...
    let guildCount = 0, memberCount = 0, channelCount = 0;
    if (this.bot.shard) {
        const counts = await this.bot.shard.getValues('ALL', ['guildCount', 'memberCount', 'channelCount']).catch(() => null);
        if (counts) {
    ...
    if (this.bot.shard) {
        description += `${LangUtils.getAndReplace('BOT_INFO_SHARDS', { shards: this.bot.shard.total_shards }, interaction.locale)} | `;
    }
    ...
    description += `  | [${this.bot.utils.getEmoji('GITHUB', interaction)} ${LangUtils.get('BOT_INFO_LINKS_GITHUB', interaction.locale)}](${GITHUB_LINK})\n\n`;

    description += `[${this.bot.utils.getEmoji('LINK_MINI', interaction)} ${LangUtils.get('BOT_INFO_LINKS_ADD', interaction.locale)}](${WEBSITE}/${process.env.RUNTIME_MODE === 'BETA' ? 'beta_' : ''}invite)`;
    description += ` | [${this.bot.utils.getEmoji('CHAT_MINI', interaction)} ${LangUtils.get('BOT_INFO_LINKS_SUPPORT', interaction.locale)}](${SUPPORT_SERVER})`;
    description += ` | [${this.bot.utils.getEmoji('TOS_MINI', interaction)} ${LangUtils.get('BOT_INFO_LINKS_TOS', interaction.locale)}](${WEBSITE}/tos)`;
    description += `\n[${this.bot.utils.getEmoji('SITE_MINI', interaction)} ${LangUtils.get('BOT_INFO_LINKS_SITE', interaction.locale)}](${WEBSITE})`;
    description += ` | [${this.bot.utils.getEmoji('DASH_MINI', interaction)} ${LangUtils.get('BOT_INFO_LINKS_DASHBOARD', interaction.locale)}](${DASHBOARD})`;
    description += ` | [${this.bot.utils.getEmoji('PRO_MINI', interaction)} ${LangUtils.get('BOT_INFO_LINKS_PRO', interaction.locale)}](${WEBSITE}/pro)`;
